...
function ProjectPreview({ index = 0, name }) {
  ...
  return (
    <div className="relative min-h-36 min-w-0 max-w-full overflow-hidden rounded-[1.25rem] border border-ice-300/10 bg-navy-950/70 p-3 sm:min-h-44 sm:p-4">
...
function ProjectCard({ index = 0, project }) {
  ...
  return (
    <GlassCard className="flex h-full min-w-0 flex-col gap-5 overflow-hidden sm:gap-6">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <div className="mt-5 flex min-w-0 max-w-full flex-wrap gap-1.5 sm:mt-6 sm:gap-2">
            ...
            {projectLinks.length > 0 && (
              <div className="flex min-w-0 flex-wrap gap-3">
                {projectLinks.map((link) => (
                  <a
                    className="max-w-full rounded-button border border-ice-300/20 px-4 py-2 text-center text-sm font-semibold leading-snug break-words text-ice-50 transition hover:border-cyan-glow/50 hover:bg-cyan-glow/10"
                    href={link.href}
                    key={link.label}
                    rel="noreferrer"
                    target="_blank"
